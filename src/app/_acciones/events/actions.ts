...
import { isUnlockValid, unlockValue, UNLOCK_MS } from '@/modules/events/domain/unlock-token'
...
const readForm = (formData: FormData) => ({
  ...
  title: campo(formData, 'title'),
  eventDate: campo(formData, 'eventDate'),
  rsvpDeadline: campo(formData, 'rsvpDeadline'),
  ...
  messageTemplate: campo(formData, 'messageTemplate'),
  venue: campo(formData, 'venue'),
})
...
export async function updateEventAction(_previous: EventActionState, formData: FormData): Promise<EventActionState> {
  ...
  const eventId = campo(formData, 'id')
  ...
  const anterior = await eventUseCases.getByIdFor(actor, eventId, { section: 'ficha' })
...
export async function deleteEventAction(
  ...
  const result = await eventUseCases.remove({
    eventId,
    confirmation: campo(formData, 'confirmation'),
  })
...
export async function setEventPrivacyAction(_previous: PrivacyState, formData: FormData): Promise<PrivacyState> {
  ...
  const eventId = campo(formData, 'eventId')
  const eventSlug = campo(formData, 'eventSlug')
  await requireEventAccess(actor, { eventId, eventSlug, section: 'ficha' })
...
export async function removeMediaAction(_previo: ContentActionState, formData: FormData): Promise<ContentActionState> {
  ...
  const resultado = await eventUseCases.media.remove(eventId, campo(formData, 'mediaId'), enUso)
  if (!resultado.ok) return { status: 'error', message: resultado.error }
