...
describe('repositorio de grupos', () => {
  it('inserta, lista por evento y encuentra por hash', async () => {
    await inRolledBackTransaction(async (tx) => {
      ...
      await repo.insert({ id: crypto.randomUUID(), eventId, label: 'Familia Rojas', seats: 4, revokedAt: null }, primero.hash)
      await repo.insert({ id: crypto.randomUUID(), eventId, label: 'Daniela Ortiz', seats: 1, revokedAt: null }, segundo.hash)
  ...
  it('revoca sin borrar y marca la apertura una sola vez', async () => {
    await inRolledBackTransaction(async (tx) => {
      ...
      await repo.insert({ id, eventId, label: 'Familia Rojas', seats: 4, revokedAt: null }, hash)
