 ...
 */
export const parseAmount = (text: string): Result<number, RegistryError> => {
  const limpio = text.trim()
  if (limpio.length === 0) return err(registryError('invalid_amount', 'Escribe un importe.'))

  const match = SHAPES.map((shape) => shape.exec(limpio)).find((m) => m !== null)
  if (!match) {
    return err(registryError('invalid_amount', `No es un importe: "${text}". Usa 1.234,50 o 1234.50.`))
  }
  ...
  if (!Number.isSafeInteger(cents)) return err(registryError('invalid_amount', 'Ese importe es demasiado grande.'))
  if (cents === 0) return err(registryError('invalid_amount', 'El importe tiene que ser mayor que cero.'))
  if (cents > MAX_AMOUNT_CENTS) return err(registryError('invalid_amount', 'Ese importe es demasiado grande.'))
