...
describe('el tema Bosque Encantado', () => {
  it('coloca las cinco ranuras', () => {
    render(<XvLucianaView {...propsDePrueba({ content: CONTENIDO_DE_MUESTRA })} />)
  ...
  it('pinta el nombre de la quinceañera desde el contenido', () => {
    render(<XvLucianaView {...propsDePrueba({ content: CONTENIDO_DE_MUESTRA })} />)
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Luciana')
  })
  ...
  it('sin contenido no revienta ni escribe «undefined»', () => {
    const { container } = render(<XvLucianaView {...propsDePrueba({ content: {} })} />)
