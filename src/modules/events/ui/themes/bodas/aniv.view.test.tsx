...
import { AnivView } from './aniv.view'
...
describe('el tema Bodas de Oro', () => {
  it('coloca las cuatro ranuras', () => {
    ...
    render(<AnivView {...propsDePrueba({ content: CONTENIDO_DE_MUESTRA })} />)
  ...
  it('pinta el titular del contenido, no uno escrito dentro', () => {
    render(
      <AnivView {...propsDePrueba({ content: { ...CONTENIDO_DE_MUESTRA, hero: { nameA: 'Prueba', nameB: 'Nombre' } } })} />,
    )
  ...
  it('sin contenido no revienta ni escribe «undefined»', () => {
    ...
    const { container } = render(<AnivView {...propsDePrueba({ content: {} })} />)
    expect(container.textContent).not.toContain('undefined')
    expect(screen.getByText('ranura-rsvp')).toBeInTheDocument()
  })

  it('en vista previa no pinta el RSVP de verdad, y lo dice', () => {
    render(<AnivView {...propsDePrueba({ content: CONTENIDO_DE_MUESTRA, preview: true })} />)
